...
const cliPath = path.join(__dirname, '../..', 'src', 're2', 'tester.js');

test('tester CLI はマッチする場合 true を返す', () => {
  const result = spawnSync(process.execPath, [cliPath, '^ne.+s$', 'news'], {
    encoding: 'utf8',
  });
...
test('tester CLI はマッチしない場合 false を返す', () => {
  const result = spawnSync(process.execPath, [cliPath, '^ne.+s$', 'spec'], {
    encoding: 'utf8',
  });
